...
const EditEmployeeScreen = () => {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput style={[styles.input, { backgroundColor: "#f0f0f0" }]} value={formData.id.toString()} editable={false} />
      ...
      {Platform.OS === "web" ? (
        <input
          type="date"
          value={formatDateForWebInput(formData.date_joined)}
          onChange={(e) => {
        ...
        <Pressable onPress={() => setShowDatePicker(true)}>
          <Text style={styles.dateInput}>{formatDate(formData.date_joined)}</Text>
        </Pressable>
      )}
      {showDatePicker && Platform.OS !== "web" && ( // Only show DateTimePicker on non-web platforms
        <DateTimePicker
          ...
          onChange={(e, selected) => {
            ...
            if (selected) handleChange("date_joined", selected);
          }}
        />
      )}
