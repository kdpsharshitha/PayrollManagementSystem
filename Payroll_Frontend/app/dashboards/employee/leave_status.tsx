...
import { getAccessToken } from "../../auth"; // Adjust the relative path as necessary

// Updated interface for a leave request object with all details
interface LeaveRequest {
  ...
  leave_type: string;
  description: string;
  created_at: string; // Date of the request
  status: string;
}
...
const EmployeeLeaveStatus: React.FC = () => {
  ...
  const fetchPendingRequests = async () => {
    ...
    try {
      ...
      const response = await axios.get("http://192.168.1.6:8000/api/leave-requests/myrequest/", {
        headers: { Authorization: `Bearer ${token}` },
      });
      // Filter for pending requests using the status field
  ...
  // Fetch processed (approved or rejected) leave requests for the current user
  ...
  // Render a single leave request item with all details
