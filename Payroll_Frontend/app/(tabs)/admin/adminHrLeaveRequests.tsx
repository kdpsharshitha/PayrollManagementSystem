...
const AdminHrLeaveRequests: React.FC = () => {
  ...
  const fetchPendingRequests = async () => {
    ...
    try {
      ...
      const response = await axios.get("http://192.168.17.49:8000/leave-requests/list/", {
        headers: { Authorization: `Bearer ${token}` },
      });

      console.log("API Response:", response.data);
      const pendingData = response.data.filter((req: LeaveRequest) => req.status === "pending");
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>HR Leave Requests (Admin Review)</Text>
...
const styles = StyleSheet.create({
  ...
  rejectButton: { backgroundColor: "#F44336", padding: 10, borderRadius: 5 },
